...
export const useAddressesStore = defineStore("address", () => {
  ...
  const fetchAddresses = async (page = 1, itemsPerPage = 10) => {
    const response = await useNuxtApp().$getCollection<ApiCollection<Address_jsonld>>(`/api/v3/customer/addresses?page=${page}&itemsPerPage=${itemsPerPage}`);
    addresses.value = response.items;
    return addresses.value;
  };

  const createAddress = async (addressData: Address_AddressInput_jsonld) => {
    const response = await useNuxtApp().$post<Address_jsonld>("/api/v3/customer/addresses", { body: addressData });
  ...
  const fetchAddressById = async (addressId: string) => {
    const response = await useNuxtApp().$get<Address_jsonld>(`/api/v3/customer/addresses/${addressId}`);

    return response;
  };

  const updateAddress = async (addressId: string, updatedData: Address_AddressInput_jsonld) => {
    const response = await useNuxtApp().$patch<Address_jsonld>(`/api/v3/customer/addresses/${addressId}`, { body: updatedData });
  ...
  const deleteAddress = async (addressId: string) => {
    await useNuxtApp().$delete(`/api/v3/customer/addresses/${addressId}`);
