...
export const useWalletTransactionStore = defineStore("wallet-transaction", () => {
  ...
  const fetchWalletTransactions = async (page = 1, itemsPerPage = 10) => {
    const { items } = await useNuxtApp().$getCollection<ApiCollection<WalletTransaction_jsonld>>(`/api/v3/customer/wallet-transactions?page=${page}&itemsPerPage=${itemsPerPage}`);
  ...
  const fetchWalletTransactionById = async (walletTransactionId: string) => {
    return await useNuxtApp().$get<WalletTransaction_jsonld>(`/api/v3/customer/wallet-transactions/${walletTransactionId}`);
  };
